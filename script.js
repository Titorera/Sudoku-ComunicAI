document.addEventListener('DOMContentLoaded', () => {
    ...
    if (empresaLogo) {
       empresaLogo.src = 'logo_crops.png';
       empresaLogo.alt = 'Logo CROPS';
    }
    ...
    // Variables para Estadísticas de la Sesión
    let sessionStats; 
    ...
    function inicializarEstadisticasSesion() {
        sessionStats = {
            playerName: currentPlayerName, // Asegurarse de que se actualice si el jugador cambia
    ...
    function finalizarEstadisticasNivel(levelKey, completed) {
        if (!sessionStats) inicializarEstadisticasSesion(); // Asegurar que sessionStats exista
        if (!currentLevelStartTime) return; // Evitar error si el nivel no se inició correctamente
        ...
        if (!theme) {
            console.error("Tema no encontrado para la clave de estadísticas:", levelKey);
            return;
        }


        sessionStats.levelsPlayedData.push({
            levelNum: theme.levelNum,
        ...
        currentLevelStartTime = null; // Resetear para el próximo inicio de nivel
    }
    ...
    function showScreen(screenElement) {
        ...
        statsScreen.classList.add('hidden'); // Ocultar pantalla de stats también
        statsTriggerArea.classList.add('hidden'); // Ocultar botón de stats
        screenElement.classList.remove('hidden');
    }

    if (continueToLevelSelectionBtn) {
        continueToLevelSelectionBtn.addEventListener('click', () => {
            currentPlayerName = playerNameInput.value.trim() || "Jugador";
            currentPlayerAge = playerAgeInput.value.trim() || "N/A";
            if (playerNameInput.value.trim() === "") { alert("Por favor, escribe tu nombre."); return; }
            if (playerAgeInput.value.trim() === "" || isNaN(parseInt(playerAgeInput.value.trim()))) { alert("Por favor, escribe una edad válida."); return; }
            
            inicializarEstadisticasSesion(); // Iniciar/Reiniciar stats para la nueva sesión
    ...
    function startGame() {
        ...
        // Si no hay sessionStats (primera carga de página o después de volver al menú ppal), inicializar.
        if (!sessionStats || (sessionStats && backToLevelsBtn && backToLevelsBtn.textContent === "Volver al Inicio")) {
             // Esta condición podría ser más precisa para cuando es realmente una nueva sesión.
             // Por ahora, si se vuelve al inicio desde el último nivel, las stats se mantienen hasta que se
             // presiona "Volver al Menú Principal" desde la pantalla de stats o se inicia un nuevo jugador.
        }
        ...
        // Lógica para texto de backToLevelsBtn
        if (backToLevelsBtn) {
            const isGameOver = vidasRestantes <= 0 && tableroDeshabilitado;
            const allLevelsWon = sessionStats && sessionStats.sessionCompletedAllLevels && tableroDeshabilitado;

            if (allLevelsWon) {
                backToLevelsBtn.textContent = "Volver al Inicio";
            } else if (isGameOver) {
                backToLevelsBtn.textContent = "Jugar de Nuevo";
            } else {
    ...
    if (backToLevelsBtn) {
        backToLevelsBtn.addEventListener('click', () => {
            ...
            // Si el nivel actual está en curso y no terminado, registrarlo como no completado
            ...
            if (buttonText === "Volver al Inicio") {
                if(statsTriggerArea && statsScreen.classList.contains('hidden')) statsTriggerArea.classList.remove('hidden'); // Mostrar botón de stats si no se vieron
                inicializarEstadisticasSesion(); 
                showScreen(playerInfoScreen);
            } else if (buttonText === "Jugar de Nuevo") {
                // sessionStats ya tiene los datos de la sesión anterior (que terminó en game over)
                // Si queremos una sesión completamente nueva para este "Jugar de Nuevo", deberíamos inicializar aquí también.
                // Por ahora, al hacer clic en el botón, se reinicia el MISMO nivel, continuando la sesión de stats.
                // Para que "Jugar de nuevo" desde Game Over inicie una nueva sesión de stats, descomentar:
                // inicializarEstadisticasSesion(); 
                startGame(); 
            } else { // "Cambiar Nivel"
    ...
    class SudokuGenerator { /* ... (como antes, sin cambios) ... */ 
    ...
    function createBoard(puzzle){ /* ... (como antes, sin cambios) ... */ 
    ...
    function handleCellInputFilter(inputElement){ /* ... (como antes, sin cambios) ... */ 
    ...
    function handleCellValidation(inputElement, r, c) { /* ... (como antes, con currentLevelMistakes++) ... */ 
        ...
        if (valueNum === currentSolution[r][c]) {
            ...
            currentLevelMistakes++; // Contar error
            vidasRestantes--;
            actualizarDisplayVidas();
    ...
    function checkIfGameWon() { /* ... (como antes, sin cambios) ... */ 
    ...
    function getCurrentBoardState(){ /* ... (como antes, sin cambios) ... */ const board=Array.from({length:N},()=>Array(N).fill(0));const cells=boardElement.querySelectorAll('.sudoku-cell');cells.forEach(cell=>{const r=parseInt(cell.dataset.row);const c=parseInt(cell.dataset.col);if(cell.classList.contains('prefilled')){board[r][c]=parseInt(cell.textContent);}else{const input=cell.querySelector('input');if(input&&input.value&&/^[1-9]$/.test(input.value)){board[r][c]=parseInt(input.value);}else{board[r][c]=0;}}});return board;}
        
    function checkSolution(){ /* ... (lógica ajustada como en la respuesta anterior) ... */ 
        if (tableroDeshabilitado) {
            const allLevelsCompleted = sessionStats && sessionStats.sessionCompletedAllLevels;
            if (vidasRestantes > 0 && allLevelsCompleted) {
                messageArea.textContent = "¡FELICIDADES! ¡Has completado todos los niveles!";
            } else if (vidasRestantes <= 0) {
                messageArea.textContent = "Juego Terminado. Intenta un nuevo juego.";
            } else if (vidasRestantes > 0) { // Ganó un nivel, esperando al siguiente
                 messageArea.textContent = "¡Nivel Superado! El siguiente nivel se cargará pronto.";
            }
            messageArea.className = vidasRestantes > 0 && allLevelsCompleted ? 'success' : (vidasRestantes > 0 ? 'success' : 'error');
            return;
        }
    ...
    function showSolution(){ /* ... (como antes, con solveBtn.disabled = true) ... */ 
        boardElement.innerHTML='';
    ...
    function generarAnalisisHabilidades(stats) {
        ...
        } else if (nivelesCompletados >= totalNivelesJugados / 2 && nivelesCompletados > 0) {
            analisis.push("¡Muy bien! Has superado varios desafíos, mostrando buena lógica y concentración.");
        } else if (nivelesCompletados > 0) {
            analisis.push("¡Buen comienzo! Cada nivel completado es un gran paso.");
        } else if (totalNivelesJugados > 0) {
            analisis.push("Los Sudokus pueden ser complicados. ¡La práctica y la paciencia son clave!");
        }
        ...
        } else if (stats.totalMistakesOverall < nivelesCompletados * 2 && nivelesCompletados > 0) {
            analisis.push("Muestras una excelente concentración cometiendo pocos errores en los niveles que superaste.");
        } else if (totalNivelesJugados > 0){
            analisis.push("Recuerda revisar bien cada número. ¡Con más atención, los errores disminuirán!");
        }
        ...
        if (stats.highestLevelCompletedNum >= 8) {
            ...
            analisis.push("Estás progresando muy bien en los niveles de dificultad media.");
        }

        if (analisis.length === 0 && totalNivelesJugados === 0) return "Aún no has jugado ningún nivel en esta sesión. ¡Anímate a empezar!";
    ...
    function displayStatistics() {
        if (!sessionStats || !sessionStats.levelsPlayedData) { // Chequeo más robusto
            if (statsPlayerInfoDiv) statsPlayerInfoDiv.innerHTML = "<p>No hay datos de juego para mostrar o la sesión no se ha iniciado correctamente.</p>";
            if (statsSummaryDiv) statsSummaryDiv.innerHTML = "";
            if (statsAnalysisP) statsAnalysisP.textContent = "";
            showScreen(statsScreen); // Mostrar la pantalla de stats incluso si está vacía
            return;
        }

        if(statsPlayerInfoDiv) statsPlayerInfoDiv.innerHTML = `
            <p><strong>Jugador:</strong> ${sessionStats.playerName || 'N/A'}</p>
            <p><strong>Edad:</strong> ${sessionStats.playerAge || 'N/A'}</p>
        `;

        let summaryHtml = `
            <p><strong>Nivel más alto completado:</strong> ${sessionStats.highestLevelCompletedNum > 0 ? `Nivel ${sessionStats.highestLevelCompletedNum}` : 'Ninguno'}</p>
            <p><strong>Total de niveles jugados en esta sesión:</strong> ${sessionStats.levelsPlayedData.length}</p>
            <p><strong>Total de niveles completados:</strong> ${sessionStats.levelsPlayedData.filter(l => l.completed).length}</p>
            <p><strong>Tiempo total de juego:</strong> ${Math.floor(sessionStats.totalTimeOverall / 60)}m ${sessionStats.totalTimeOverall % 60}s</p>
            <p><strong>Total de errores cometidos:</strong> ${sessionStats.totalMistakesOverall}</p>
        `;
        
        if (sessionStats.levelsPlayedData.length > 0) {
            summaryHtml += "<h4>Detalle por Nivel Jugado:</h4><ul>";
            sessionStats.levelsPlayedData.forEach(level => {
                summaryHtml += `<li>Nivel ${level.levelNum} (${level.levelName}): ${level.completed ? 'Completado ✔' : 'No Completado ✖'} - Tiempo: ${level.timeTakenSeconds}s - Errores: ${level.mistakesMade}</li>`;
            });
            summaryHtml += "</ul>";
        }
    ...
    currentDifficultyKey = levelKeys[0];
    inicializarEstadisticasSesion(); // Siempre inicializar al cargar la página
    showScreen(playerInfoScreen);
});